...
public class EnumLookupTest {
    ...
    public void testNonAnnotationEnum() {
        try {
            ...
            // 没有使用注解，所以不在代理类管理范围内
            NonAnnotationEnum instance = EnumLookup.INSTANCE.getEnum(NonAnnotationEnum.class, 1);
            assertTrue(Objects.isNull(instance));

            // 没在管理范围内，所以代理中的size应该是0
            assertEquals(0, EnumLookup.INSTANCE.sizeOf(NonAnnotationEnum.class));
        } catch (Exception e) {
    ...
    public void testNonInstanceEnum() {
        try {
            ...
            // 没有实例，所以获取结果也应该是null
            NonInstanceEnum instance = EnumLookup.INSTANCE.getEnum(NonInstanceEnum.class, 1);
            assertTrue(Objects.isNull(instance));

            // 没有任何实例，所以代理中的size应该是0
            assertEquals(0, EnumLookup.INSTANCE.sizeOf(NonInstanceEnum.class));
        } catch (Exception e) {
            fail("No exceptions should be thrown");
        }
    }
    ...
    public void testNonRepeatEnum() {
        try {
            ...
            // 有实例了，应该可以成功获取
            NonRepeatEnum instance = EnumLookup.INSTANCE.getEnum(NonRepeatEnum.class, 1);
            assertTrue(Objects.nonNull(instance));
            assertEquals(instance, NonRepeatEnum.ONE);

            // size也应该是真正的实际数量
            assertEquals(2, EnumLookup.INSTANCE.sizeOf(NonRepeatEnum.class));
        } catch (Exception e) {
            fail("No exceptions should be thrown");
        }
    }
    ...
    public void testRepeatEnum() {
        try {
            ...
            // 注册错误，所以也应该获取不到
            RepeatEnum instance = EnumLookup.INSTANCE.getEnum(RepeatEnum.class, 1);
            assertTrue(Objects.isNull(instance));

            // 注册错误，所以代理中的size也是0
            assertEquals(0, EnumLookup.INSTANCE.sizeOf(RepeatEnum.class));
        } catch (Exception e) {
            fail("No exceptions should be thrown");
        }
    }
