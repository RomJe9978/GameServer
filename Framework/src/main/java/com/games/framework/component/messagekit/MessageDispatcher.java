...
import com.games.framework.component.packetkit.PacketHandler;
import com.games.framework.component.packetkit.PacketListener;
...
public enum MessageDispatcher {
    ...
    /**
     * 指定"Message"对应最终“处理方法”的信息
     * key：消息标识Id（非实例Id），value：最终处理方法的信息封装
     */
    private final Map<Integer, MessageListenerEntry> messageListenersMap = new HashMap<>();
    ...
    /**
     * 派发消息包，调用监听该消息的最终处理方法
     * <p> 如果针对指定消息包，没有任何监听方法需要执行，则认为成功，返回{@code true}
     *
     * @param messageId 消息包唯一标识id（注意不是实例id，而是同一消息的标识id）
     * @param param1    最终处理方法所需要的参数，不允许为{@code null}
     * @param param2    最终处理方法所需要的参数，不允许为{@code null}
     */
    public void dispatch(int messageId, @NonNull Object param1, @NonNull Object param2) {
        MessageListenerEntry entry = this.messageListenersMap.get(messageId);
        ...
        try {
            ...
            Log.FRAME.warn("[Message] handler:{} handle message id:{} exception!", entry, messageId, e);
        }
    }

    /**
     * 扫描指定包中的所有消息包处理类（使用了{@link MessageHandler}标识的类），
     * 注册这些处理类内的所有消息包处理方法（使用了{@link MessageListener}标识的方法）。
     *
     * @param packageName 如果扫描不到任何处理类，返回成功。
     * @return 任何错误或者异常返回{@code false}，失败信息在{@link BoolResult#message()}中。
     */
    public BoolResult registerMessageListener(String packageName) {
        ...
        return this.registerMessageListener(classList);
    }

    /**
     * 注册一组处理类中的，所有最终处理方法（扫描{@link MessageHandler}注解）。
     *
     * @param classList 一组处理类，如果列表为空，则返回成功。
     * @return 任何错误或者异常返回{@code false}，失败信息在{@link BoolResult#message()}中。
     */
    public BoolResult registerMessageListener(List<Class<?>> classList) {
        if (EmptyUtil.isEmpty(classList)) {
            return BoolResult.success();
        }

        for (Class<?> clazz : classList) {
            BoolResult singleResult = registerMessageListener(clazz);
    ...
    /**
     * 注册单个处理类中的，所有处理方法（扫描{@link MessageListener}注解）。
     *
     * @param handlerClass 单个处理类，不允许为{@code null}。
     * @return 任何错误或者异常返回{@code false}，失败信息在{@link BoolResult#message()}中。
     */
    public BoolResult registerMessageListener(Class<?> handlerClass) {
     ...
     * @param method       最终的处理方法，不允许为{@code null}。必须标识{@link MessageListener}注解。
     * @return 任何错误或者异常返回{@code false}，失败信息在{@link BoolResult#message()}中。
     */
    private BoolResult registerListenMethod(Class<?> handlerClass, Method method) {
        ...
        try {
            ...
            // 注册即可
            MessageListenerEntry newEntry = MessageListenerEntry.of(handlerClass, method, annotation, consumer);
            this.messageListenersMap.put(annotation.value(), newEntry);
